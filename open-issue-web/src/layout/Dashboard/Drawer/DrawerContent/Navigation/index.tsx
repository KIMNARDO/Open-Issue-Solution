...
const Navigation = () => {
  ...
  const { data: menuItem } = useMenuList();
  const { setMenuList } = useStackBar();
  const { user } = useAuth();

  useLayoutEffect(() => {
    if (menuItem && user) {
      // 메뉴명 태그제거 (임시) ** 추후 db에서 메뉴명 변경 예정
      menuItem.forEach((item) => {
        item.name = item.name.replaceAll(tagRegex, '');
        item.childMenu &&
          item.childMenu.forEach((child) => {
            child.name = child.name.replaceAll(tagRegex, '');
            child.childMenu = [];
          });
      });
  ...
  return (
    <Box
      sx={{
        ...
        display: 'flex' //isHorizontal ? { xs: 'block', lg: 'flex' } : 'block'
      }}
    >
      {navGroups}
    </Box>
